...
PINS = [
    ...
    ("mock.ts", "sidecar.py", 1353, "Partial update: an OMITTED field", "the metadata.set tri-state"),
    ("mock.ts", "sidecar.py", 1447, "under an EXPLICIT", "dedup.scan requires codex_home"),
    ("mock.ts", "sidecar.py", 1574, "SessionStoreKind.UNKNOWN", "the edge forces every target to UNKNOWN"),
    ("mock.ts", "sidecar.py", 1608, "result.executed and result.manifest_path", "only an applied run is recorded"),
    ...
    ("types.ts", "sidecar.py", 1304, "research.extract_entities", "extraction routes through the same backend"),
    ("types.ts", "sidecar.py", 1337, "return {", "the annotation DTO"),  # weak
    ("types.ts", "sidecar.py", 1325, "the absorbed csm annotation layer", "annotations are local-only by design"),
    ("types.ts", "sidecar.py", 1346, "Un-annotated reads back as an EMPTY", "is_empty rather than an error"),
    ("types.ts", "sidecar.py", 1352, "def _metadata_set", "the metadata.set surface"),
    ("types.ts", "sidecar.py", 1353, "Partial update: an OMITTED field", "the tri-state is the whole point"),
    ("types.ts", "sidecar.py", 1359, "if tags is not None and not isinstance", "a non-list tags is -32602"),
    ("types.ts", "sidecar.py", 1383, "def _metadata_search", "the metadata.search surface"),
    ("types.ts", "sidecar.py", 1385, "With neither filter the result is empty", "a blank query dumps nothing"),
    ("types.ts", "sidecar.py", 1395, "return [", "the search row DTO"),  # weak
    ("types.ts", "sidecar.py", 1415, '{"tag": _clean(tag)', "one entry of the tag facet"),
    ("types.ts", "sidecar.py", 1418, "Codex physical copies -> one logical session", "dedup is a view, never a delete"),
    ("types.ts", "sidecar.py", 1421, "The paths it returns are LOCAL filesystem paths", "display-sensitive, absent from MetadataView"),
    ("types.ts", "sidecar.py", 1434, "return {", "the DedupSession DTO"),  # weak
    ("types.ts", "sidecar.py", 1460, "return {", "the dedup.scan result"),  # weak
    ("types.ts", "sidecar.py", 1475, "the ONLY destructive surface", "why the client never sends a preview back"),
    ("types.ts", "sidecar.py", 1491, '"session_id": copy.session_id', "one session file inside a plan"),
    ("types.ts", "sidecar.py", 1492, "copy.store_kind.value", "a real enum serialized via .value"),
    ("types.ts", "sidecar.py", 1495, "def _preview_dto", "the plan preview DTO"),
    ("types.ts", "sidecar.py", 1503, '"blocked": [{"target"', "a refused target and its reason"),
    ("types.ts", "sidecar.py", 1505, '"warnings": [{"severity"', "the warning DTO"),
    ("types.ts", "sidecar.py", 1507, '"plan": [{"session_id"', "the planned moves"),
    ("types.ts", "sidecar.py", 1516, "return {", "the execute/restore result"),  # weak
    ("types.ts", "sidecar.py", 1538, "def _maintenance_plan", "maintenance.plan is pure"),
    ("types.ts", "sidecar.py", 1548, '(("store_root", store_root)', "both roots refused if UNC or relative"),
    ("types.ts", "sidecar.py", 1554, '_req_str(params, "action")', "an unknown action is -32602"),
    ("types.ts", "sidecar.py", 1562, "raw_targets, list) or not raw_targets", "targets must be a non-empty array"),
    ("types.ts", "sidecar.py", 1564, "targets = []", "how the edge builds each target"),  # weak
    ("types.ts", "sidecar.py", 1568, 'item.get("file_path")', "file_path is required per target"),
    ("types.ts", "sidecar.py", 1572, 'item.get("session_id"', "session_id defaults to empty"),
    ("types.ts", "sidecar.py", 1574, "SessionStoreKind.UNKNOWN", "the edge forces every target to UNKNOWN"),
    ("types.ts", "sidecar.py", 1575, 'item.get("size_bytes", 0) or 0', "size_bytes defaults to 0"),
    ("types.ts", "sidecar.py", 1587, "def _maintenance_execute", "the execute surface"),
    ("types.ts", "sidecar.py", 1605, "Consumed only once the engine ACCEPTED it", "a refused confirmation is correctable"),
    ("types.ts", "sidecar.py", 1612, "def _maintenance_restore", "the restore surface"),
    ("types.ts", "sidecar.py", 1617, "_reject_nonlocal_path(manifest_path", "manifest_path refused if UNC or relative"),
    ("types.ts", "sidecar.py", 1447, "under an EXPLICIT", "dedup.scan requires codex_home"),
...
PY_PINS = [
    ...
    ("claude_code.py", "sidecar.py", 1652, '"provider": meta.adapter', "the adapter label surfaced as `provider`"),
    ...
    ("grok.py", "sidecar.py", 1652, '"provider": meta.adapter', "the adapter label surfaced as `provider`"),
]
