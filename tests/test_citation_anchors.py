...
PINS = [
    ...
    # --------------------------------------------------------------------- sidecar.py (65)
    ("mock.ts", "sidecar.py", 350, "JSON-RPC error envelope", "the {code, message} envelope"),
    ...
    ("mock.ts", "sidecar.py", 905, '_reject_nonlocal_path(index_path', "the index_path guard the mock reproduces"),
    ("mock.ts", "sidecar.py", 958, "Returns immediately", "the build reply means ACCEPTED, not finished"),
    ("mock.ts", "sidecar.py", 999, "if not sessions_root and not grok_root", "at least one source root must be named"),
    ("mock.ts", "sidecar.py", 1010, '(sessions_root, "sessions_root")', "each named root refused if UNC or relative"),
    ("mock.ts", "sidecar.py", 1173, "if snapshot is None", "no job yet is a state, not an error"),
    ("mock.ts", "sidecar.py", 1178, 'requested is not None and requested != snapshot["job_id"]', "a stale job_id is refused, not answered"),
    ("mock.ts", "sidecar.py", 1182, '"sessions_root": _clean(snapshot["sessions_root"])', "build_status reports the root the job started with"),
    ...
    ("types.ts", "sidecar.py", 990, 'sessions_root = _opt_str(params, "sessions_root")', "every corpus.build param is optional"),
    ("types.ts", "sidecar.py", 999, "if not sessions_root and not grok_root", "the only refusal: NEITHER root named"),
    ("mock.ts", "sidecar.py", 1177, '"state": "idle"', "idle before any build, rather than erroring"),
    ("mock.ts", "sidecar.py", 702, "self.research_backend", "the MockBackend fallback"),
    ("mock.ts", "sidecar.py", 1592, "Partial update: an OMITTED field", "the metadata.set tri-state"),
    ("mock.ts", "sidecar.py", 1686, "Scan the known Codex stores under an EXPLICIT", "dedup.scan requires codex_home"),
    ("mock.ts", "sidecar.py", 1813, "SessionStoreKind.UNKNOWN", "the edge forces every target to UNKNOWN"),
    ("mock.ts", "sidecar.py", 1847, "result.executed and result.manifest_path", "only an applied run is recorded"),
    ("types.ts", "sidecar.py", 914, '"index_path": _clean(index_path)', "corpus.create returns {index_path, created}"),
    ("types.ts", "sidecar.py", 357, "def _clean", "the hidden-unicode strip, via _sanitize_tree"),
    ("types.ts", "sidecar.py", 958, "Returns immediately", "the build reply means ACCEPTED, not finished"),
    ("types.ts", "sidecar.py", 1154, "def _corpus_build_status", "the poll-safe status surface"),
    ("types.ts", "sidecar.py", 737, '"research.synthesize"', "the research methods exist on the engine"),
    ("types.ts", "sidecar.py", 702, "self.research_backend", "the MockBackend fallback"),
    ("types.ts", "sidecar.py", 1543, "research.extract_entities(views, self.research_backend)", "extraction routes through the same backend"),
    ("types.ts", "sidecar.py", 1576, "return {", "the annotation DTO"),  # weak
    ("types.ts", "sidecar.py", 1564, "the absorbed csm annotation layer", "annotations are local-only by design"),
    ("types.ts", "sidecar.py", 1585, "Un-annotated reads back as an EMPTY", "is_empty rather than an error"),
    ("types.ts", "sidecar.py", 1591, "def _metadata_set", "the metadata.set surface"),
    ("types.ts", "sidecar.py", 1592, "Partial update: an OMITTED field", "the tri-state is the whole point"),
    ("types.ts", "sidecar.py", 1598, "if tags is not None and not isinstance", "a non-list tags is -32602"),
    ("types.ts", "sidecar.py", 1622, "def _metadata_search", "the metadata.search surface"),
    ("types.ts", "sidecar.py", 1624, "With neither filter the result is empty", "a blank query dumps nothing"),
    ("types.ts", "sidecar.py", 1634, "return [", "the search row DTO"),  # weak
    ("types.ts", "sidecar.py", 1654, '{"tag": _clean(tag)', "one entry of the tag facet"),
    ("types.ts", "sidecar.py", 1657, "Codex physical copies -> one logical session", "dedup is a view, never a delete"),
    ("types.ts", "sidecar.py", 1660, "The paths it returns are LOCAL filesystem paths", "display-sensitive, absent from MetadataView"),
    ("types.ts", "sidecar.py", 1673, "return {", "the DedupSession DTO"),  # weak
    ("types.ts", "sidecar.py", 1699, "return {", "the dedup.scan result"),  # weak
    ("types.ts", "sidecar.py", 1714, "the ONLY destructive surface", "why the client never sends a preview back"),
    ("types.ts", "sidecar.py", 1730, '"session_id": copy.session_id', "one session file inside a plan"),
    ("types.ts", "sidecar.py", 1731, "copy.store_kind.value", "a real enum serialized via .value"),
    ("types.ts", "sidecar.py", 1734, "def _preview_dto", "the plan preview DTO"),
    ("types.ts", "sidecar.py", 1742, '"blocked": [{"target"', "a refused target and its reason"),
    ("types.ts", "sidecar.py", 1744, '"warnings": [{"severity"', "the warning DTO"),
    ("types.ts", "sidecar.py", 1746, '"plan": [{"session_id"', "the planned moves"),
    ("types.ts", "sidecar.py", 1755, "return {", "the execute/restore result"),  # weak
    ("types.ts", "sidecar.py", 1777, "def _maintenance_plan", "maintenance.plan is pure"),
    ("types.ts", "sidecar.py", 1787, '(("store_root", store_root)', "both roots refused if UNC or relative"),
    ("types.ts", "sidecar.py", 1793, '_req_str(params, "action")', "an unknown action is -32602"),
    ("types.ts", "sidecar.py", 1801, "raw_targets, list) or not raw_targets", "targets must be a non-empty array"),
    ("types.ts", "sidecar.py", 1803, "targets = []", "how the edge builds each target"),  # weak
    ("types.ts", "sidecar.py", 1807, 'item.get("file_path")', "file_path is required per target"),
    ("types.ts", "sidecar.py", 1811, 'item.get("session_id"', "session_id defaults to empty"),
    ("types.ts", "sidecar.py", 1813, "SessionStoreKind.UNKNOWN", "the edge forces every target to UNKNOWN"),
    ("types.ts", "sidecar.py", 1814, 'item.get("size_bytes", 0) or 0', "size_bytes defaults to 0"),
    ("types.ts", "sidecar.py", 1826, "def _maintenance_execute", "the execute surface"),
    ("types.ts", "sidecar.py", 1844, "Consumed only once the engine ACCEPTED it", "a refused confirmation is correctable"),
    ("types.ts", "sidecar.py", 1851, "def _maintenance_restore", "the restore surface"),
    ("types.ts", "sidecar.py", 1856, "_reject_nonlocal_path(manifest_path", "manifest_path refused if UNC or relative"),
    ("types.ts", "sidecar.py", 1686, "Scan the known Codex stores under an EXPLICIT", "dedup.scan requires codex_home"),
    ("types.ts", "sidecar.py", 350, "JSON-RPC error envelope", "the {code, message} envelope"),
    ("types.ts", "sidecar.py", 799, "params must be an object", "-32602, wrong params"),
    ("types.ts", "sidecar.py", 792, "Internal error", "-32603, an unhandled engine fault"),
    ("types.ts", "sidecar.py", 293, "CORPUS_NOT_INDEXED", "-32000, no corpus attached"),
    ("types.ts", "sidecar.py", 294, "THREAD_NOT_FOUND", "-32001"),
    ("types.ts", "sidecar.py", 295, "DB_BUSY", "-32002, sqlite lock/busy"),
    ("types.ts", "sidecar.py", 296, "the safety model REFUSED", "the maintenance-refused code"),
    ...
    # silent pass.
    ("mock.ts", "sidecar.py", 1306, "def _export_plan", "the dry-run surface the mock mirrors"),
    ...
    ("types.ts", "sidecar.py", 1277, "def _export_mode", "absent means full; empty and null are -32602"),
    ("types.ts", "sidecar.py", 1295, "def _export_scrub", "a real bool or -32602, never a truthiness guess"),
    ("types.ts", "sidecar.py", 1306, "def _export_plan", "the pre-write warning surface"),
    ("types.ts", "sidecar.py", 2039, "def _project_export_run", "the ExportResult DTO, scan forwarded on failure"),
