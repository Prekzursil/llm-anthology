...
PINS = [
    ...
    ("mock.ts", "sidecar.py", 858, "if not sessions_root and not grok_root", "at least one source root must be named"),
    ("mock.ts", "sidecar.py", 865, '(sessions_root, "sessions_root")', "each named root refused if UNC or relative"),
    ("mock.ts", "sidecar.py", 991, "if snapshot is None", "no job yet is a state, not an error"),
    ("mock.ts", "sidecar.py", 996, 'requested is not None and requested != snapshot["job_id"]', "a stale job_id is refused, not answered"),
    ("mock.ts", "sidecar.py", 1000, '"sessions_root": _clean(snapshot["sessions_root"])', "build_status reports the root the job started with"),
    ...
    ("types.ts", "sidecar.py", 856, 'sessions_root = _opt_str(params, "sessions_root")', "every corpus.build param is optional"),
    ("types.ts", "sidecar.py", 858, "if not sessions_root and not grok_root", "the only refusal: NEITHER root named"),
    ("mock.ts", "sidecar.py", 995, '"state": "idle"', "idle before any build, rather than erroring"),
    ("mock.ts", "sidecar.py", 587, "self.research_backend", "the MockBackend fallback"),
    ("mock.ts", "sidecar.py", 1335, "Partial update: an OMITTED field", "the metadata.set tri-state"),
    ("mock.ts", "sidecar.py", 1429, "under an EXPLICIT", "dedup.scan requires codex_home"),
    ("mock.ts", "sidecar.py", 1556, "SessionStoreKind.UNKNOWN", "the edge forces every target to UNKNOWN"),
    ("mock.ts", "sidecar.py", 1590, "result.executed and result.manifest_path", "only an applied run is recorded"),
    ...
    ("types.ts", "sidecar.py", 972, "def _corpus_build_status", "the poll-safe status surface"),
    ...
    ("types.ts", "sidecar.py", 1286, "research.extract_entities", "extraction routes through the same backend"),
    ("types.ts", "sidecar.py", 1319, "return {", "the annotation DTO"),  # weak
    ("types.ts", "sidecar.py", 1307, "the absorbed csm annotation layer", "annotations are local-only by design"),
    ("types.ts", "sidecar.py", 1328, "Un-annotated reads back as an EMPTY", "is_empty rather than an error"),
    ("types.ts", "sidecar.py", 1334, "def _metadata_set", "the metadata.set surface"),
    ("types.ts", "sidecar.py", 1335, "Partial update: an OMITTED field", "the tri-state is the whole point"),
    ("types.ts", "sidecar.py", 1341, "if tags is not None and not isinstance", "a non-list tags is -32602"),
    ("types.ts", "sidecar.py", 1365, "def _metadata_search", "the metadata.search surface"),
    ("types.ts", "sidecar.py", 1367, "With neither filter the result is empty", "a blank query dumps nothing"),
    ("types.ts", "sidecar.py", 1377, "return [", "the search row DTO"),  # weak
    ("types.ts", "sidecar.py", 1397, '{"tag": _clean(tag)', "one entry of the tag facet"),
    ("types.ts", "sidecar.py", 1400, "Codex physical copies -> one logical session", "dedup is a view, never a delete"),
    ("types.ts", "sidecar.py", 1403, "The paths it returns are LOCAL filesystem paths", "display-sensitive, absent from MetadataView"),
    ("types.ts", "sidecar.py", 1416, "return {", "the DedupSession DTO"),  # weak
    ("types.ts", "sidecar.py", 1442, "return {", "the dedup.scan result"),  # weak
    ("types.ts", "sidecar.py", 1457, "the ONLY destructive surface", "why the client never sends a preview back"),
    ("types.ts", "sidecar.py", 1473, '"session_id": copy.session_id', "one session file inside a plan"),
    ("types.ts", "sidecar.py", 1474, "copy.store_kind.value", "a real enum serialized via .value"),
    ("types.ts", "sidecar.py", 1477, "def _preview_dto", "the plan preview DTO"),
    ("types.ts", "sidecar.py", 1485, '"blocked": [{"target"', "a refused target and its reason"),
    ("types.ts", "sidecar.py", 1487, '"warnings": [{"severity"', "the warning DTO"),
    ("types.ts", "sidecar.py", 1489, '"plan": [{"session_id"', "the planned moves"),
    ("types.ts", "sidecar.py", 1498, "return {", "the execute/restore result"),  # weak
    ("types.ts", "sidecar.py", 1520, "def _maintenance_plan", "maintenance.plan is pure"),
    ("types.ts", "sidecar.py", 1530, '(("store_root", store_root)', "both roots refused if UNC or relative"),
    ("types.ts", "sidecar.py", 1536, '_req_str(params, "action")', "an unknown action is -32602"),
    ("types.ts", "sidecar.py", 1544, "raw_targets, list) or not raw_targets", "targets must be a non-empty array"),
    ("types.ts", "sidecar.py", 1546, "targets = []", "how the edge builds each target"),  # weak
    ("types.ts", "sidecar.py", 1550, 'item.get("file_path")', "file_path is required per target"),
    ("types.ts", "sidecar.py", 1554, 'item.get("session_id"', "session_id defaults to empty"),
    ("types.ts", "sidecar.py", 1556, "SessionStoreKind.UNKNOWN", "the edge forces every target to UNKNOWN"),
    ("types.ts", "sidecar.py", 1557, 'item.get("size_bytes", 0) or 0', "size_bytes defaults to 0"),
    ("types.ts", "sidecar.py", 1569, "def _maintenance_execute", "the execute surface"),
    ("types.ts", "sidecar.py", 1587, "Consumed only once the engine ACCEPTED it", "a refused confirmation is correctable"),
    ("types.ts", "sidecar.py", 1594, "def _maintenance_restore", "the restore surface"),
    ("types.ts", "sidecar.py", 1599, "_reject_nonlocal_path(manifest_path", "manifest_path refused if UNC or relative"),
    ("types.ts", "sidecar.py", 1429, "under an EXPLICIT", "dedup.scan requires codex_home"),
...
PY_PINS = [
    ...
    ("sidecar.py", "corpus.py", 274, "sqlite3.connect(path)", "the DEFAULT check_same_thread=True"),
    ("sidecar.py", "corpus.py", 263, "PRAGMA journal_mode=WAL", "WAL is what lets both connections share the file"),
    ("sidecar.py", "loaders.py", 419, "corpus.open_index(index_path)", "the worker opens its OWN connection"),
    ("sidecar.py", "loaders.py", 430, "conn.close()", "...and closes it, inside the worker thread"),
    ("sidecar.py", "index.py", 171, "if progress is not None", "the only cooperative abort point in the stack"),
    ("sidecar.py", "loaders.py", 319, "def load_corpus", "load_corpus DOES accept a progress callback"),
    ("sidecar.py", "loaders.py", 428, "progress=progress", "...and DOES forward it — the old premise is dead"),
    ("sidecar.py", "sidecar.py", 928, "loaders.load_corpus(", "the worker call that passes none"),
    ("sidecar.py", "sidecar.py", 876, 'codex_home = _opt_str(params, "codex_home")', "codex_home is read OPTIONAL, never required"),
    ...
    ("sidecar.py", "loaders.py", 421, "_persist_graph(conn, result)", "the graph commits BEFORE the long ingest"),
    ("sidecar.py", "loaders.py", 366, "result = corpus.Corpus()", "load_corpus starts from a FRESH Corpus"),
    ("sidecar.py", "loaders.py", 725, "upsert_thread(conn", "the run is UPSERTed into a previous build's tables"),
    ...
    # ------------------------------------------------------------------------ cli.py (4)
    ("cli.py", "loaders.py", 319, "def load_corpus", "load_corpus DOES accept a progress callback"),
    ("cli.py", "loaders.py", 428, "progress=progress", "...and forwards it after every committed chunk"),
    ...
    ("claude_code.py", "sidecar.py", 1634, '"provider": meta.adapter', "the adapter label surfaced as `provider`"),
    ...
    ("grok.py", "sidecar.py", 1634, '"provider": meta.adapter', "the adapter label surfaced as `provider`"),
]
